...
void GraphicsDebugMarkers::beginRegion(const std::string& name, const DebugColor& color) {
    ...
    if (m_glPushDebugGroupFunc) {
        ...
        unsigned int colorInt = 
            (static_cast<unsigned int>(color.r * 255) << 24) |
            (static_cast<unsigned int>(color.g * 255) << 16) |
            (static_cast<unsigned int>(color.b * 255) << 8) |
            static_cast<unsigned int>(color.a * 255);
        
        typedef void (*PushDebugGroupFunc)(GLenum source, GLuint id, GLsizei length, const GLchar* message);
        auto pushFunc = reinterpret_cast<PushDebugGroupFunc>(m_glPushDebugGroupFunc);
        pushFunc(GL_DEBUG_SOURCE_APPLICATION, colorInt, static_cast<GLsizei>(name.length()), name.c_str());
    } else {
...
void GraphicsDebugMarkers::insertMarker(const std::string& message) {
    ...
    if (m_glDebugMessageInsertFunc) {
        ...
        insertFunc(GL_DEBUG_SOURCE_APPLICATION,
...
void GraphicsDebugMarkers::labelObject(unsigned int objectType, unsigned int objectID, const std::string& label) {
    if (!m_enabled || !m_hasDebugExtension) return;
    
    if (m_glObjectLabelFunc) {
        typedef void (*ObjectLabelFunc)(GLenum identifier, GLuint name,
                                       GLsizei length, const GLchar* label);
        auto labelFunc = reinterpret_cast<ObjectLabelFunc>(m_glObjectLabelFunc);
        labelFunc(objectType, objectID,
                 static_cast<GLsizei>(label.length()), label.c_str());
    } else {
        // Fallback
        if (GLEW_KHR_debug) {
            glObjectLabel(objectType, objectID,
                        static_cast<GLsizei>(label.length()), label.c_str());
        }
    }
}
