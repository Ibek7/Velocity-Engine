...
Animator::Animator()
    : currentFrame(0), frameTimer(0), playing(false), finished(false) {}
...
float Ease::elasticIn(float t) {
    if (t == 0.0f) return 0.0f;
    if (t == 1.0f) return 1.0f;
    return -std::pow(2.0f, 10.0f * t - 10.0f) * std::sin((t * 10.0f - 10.75f) * (2.0f * M_PI) / 3.0f);
}
...
float Ease::elasticInOut(float t) {
    ...
    if (t < 0.5f) {
        return -(std::pow(2.0f, 20.0f * t - 10.0f) * std::sin((20.0f * t - 11.125f) * (2.0f * M_PI) / 4.5f)) / 2.0f;
    }
    return (std::pow(2.0f, -20.0f * t + 10.0f) * std::sin((20.0f * t - 11.125f) * (2.0f * M_PI) / 4.5f)) / 2.0f + 1.0f;
}
...
float Tween<float>::applyEasing(float t) {
    switch (easeType) {
        case EaseType::Linear: return Ease::linear(t);
        case EaseType::QuadIn: return Ease::quadIn(t);
        case EaseType::QuadOut: return Ease::quadOut(t);
        case EaseType::QuadInOut: return Ease::quadInOut(t);
        case EaseType::CubicIn: return Ease::cubicIn(t);
        case EaseType::CubicOut: return Ease::cubicOut(t);
        case EaseType::CubicInOut: return Ease::cubicInOut(t);
        case EaseType::SineIn: return Ease::sineIn(t);
        case EaseType::SineOut: return Ease::sineOut(t);
        case EaseType::SineInOut: return Ease::sineInOut(t);
        default: return t;
    }
}
...
template<>
Math::Vector2D Tween<Math::Vector2D>::interpolate(const Math::Vector2D& a, const Math::Vector2D& b, float t) {
    return a.lerp(b, t);
}
