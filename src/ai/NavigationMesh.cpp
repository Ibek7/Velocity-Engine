...
void NavMeshNode::addVertex(const JJM::Math::Vector2D& vertex) { vertices.push_back(vertex); }
...
NavMeshEdge::NavMeshEdge(int id, const JJM::Math::Vector2D& start, const JJM::Math::Vector2D& end)
...
NavMeshPath NavMesh::findPath(int startNodeId, int endNodeId) const {
    ...
    if (startNodeId < 0 || endNodeId < 0 || startNodeId >= static_cast<int>(nodes.size()) ||
...
void NavMesh::disableNode(int nodeId) { disabledNodes[nodeId] = true; }

void NavMesh::enableNode(int nodeId) { disabledNodes.erase(nodeId); }
...
void NavMesh::reconstructPath(const std::unordered_map<int, int>& cameFrom, int current,
...
void NavMeshBuilder::createPolygons(NavMesh& mesh, const std::vector<Cell>& grid, int width,
...
void NavMeshBuilder::connectNeighborNodes(NavMesh& mesh, const std::vector<Cell>& grid, int width,
                                          int height) {
    int nodeId = 0;
            ...
            if (grid[idx].walkable) {
                ...
                if (x + 1 < width && grid[idx + 1].walkable) {
                    ...
                    if (current && current->getVertices().size() >= 2) {
                        int edgeId =
                            mesh.addEdge(current->getVertices()[1], current->getVertices()[2]);
                ...
                if (y + 1 < height && grid[idx + width].walkable) {
                    ...
                    if (current && current->getVertices().size() >= 3) {
                        ...
                        int edgeId =
                            mesh.addEdge(current->getVertices()[2], current->getVertices()[3]);
...
void NavMeshAgent::setPosition(const JJM::Math::Vector2D& pos) { position = pos; }
...
void NavMeshAgent::pause() { paused = true; }

void NavMeshAgent::resume() { paused = false; }
...
void LocalAvoidance::removeAgent(int id) { agents.erase(id); }
...
JJM::Math::Vector2D LocalAvoidance::computeAvoidanceVelocity(
    int agentId, const JJM::Math::Vector2D& preferredVelocity, float maxSpeed) {
...
std::vector<LocalAvoidance::Line> LocalAvoidance::computeORCALines(
    int agentId, const JJM::Math::Vector2D& preferredVelocity) {
    std::vector<LocalAvoidance::Line> lines;
...
JJM::Math::Vector2D LocalAvoidance::linearProgram(const std::vector<LocalAvoidance::Line>& lines,
