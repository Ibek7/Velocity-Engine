...
namespace Easing {
    ...
    inline float easeOutElastic(float t) {
        ...
        return std::pow(2.0f, -10.0f * t) * 
               std::sin((t - p / 4.0f) * (2.0f * M_PI) / p) + 1.0f;
    }
    inline float easeInOutElastic(float t) {
        ...
        if (t < 0.5f) {
            ...
            return 0.5f * std::pow(2.0f, -20.0f * t + 10.0f) *
                   std::sin((2.0f * t - 1.0f - p / 4.0f) * (2.0f * M_PI) / p) + 1.0f;
        }
    }
    ...
    inline float easeOutBack(float t) {
        float c1 = 1.70158f;
        return 1.0f + (c1 + 1.0f) * std::pow(t - 1.0f, 3.0f) + 
...
public:
     ...
     */
    static Vector2D quadratic(const Vector2D& p0, const Vector2D& p1, 
                             const Vector2D& p2, float t) {
        float u = 1.0f - t;
        float tt = t * t;
        float uu = u * u;
     ...
     * @return Point on curve
     */
    static Vector2D cubic(const Vector2D& p0, const Vector2D& p1,
                         const Vector2D& p2, const Vector2D& p3, float t) {
        float u = 1.0f - t;
        float tt = t * t;
        float uu = u * u;
        float uuu = uu * u;
...
public:
     ...
     */
    static Vector2D interpolate(const Vector2D& p0, const Vector2D& p1,
                                const Vector2D& p2, const Vector2D& p3, float t) {
        float t2 = t * t;
        float t3 = t2 * t;
...
public:
     ...
     * @return Point on spline
     */
    static Vector2D interpolate(const Vector2D& p0, const Vector2D& t0,
                                const Vector2D& p1, const Vector2D& t1, float t) {
        float t2 = t * t;
        float t3 = t2 * t;
