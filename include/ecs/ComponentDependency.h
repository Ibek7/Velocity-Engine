...
class ComponentDependencyRegistry {
   public:
    ...
    void registerRequiredDependency(const std::string& description = "") {
        registerDependency(typeid(ComponentType), typeid(RequiredType), DependencyType::Required,
                           description);
    }
    ...
    void registerOptionalDependency(const std::string& description = "") {
        registerDependency(typeid(ComponentType), typeid(OptionalType), DependencyType::Optional,
                           description);
    }
...
class DependencyGraph {
   ...
   private:
    struct Node {
        ...
        Node() : type(typeid(void)) {}
        Node(std::type_index t) : type(t) {}
    };

    std::unordered_map<std::type_index, Node> m_nodes;

    bool dfsHasCycle(std::type_index current, std::unordered_set<std::type_index>& visited,
                     std::unordered_set<std::type_index>& recursionStack,
                     std::vector<std::type_index>& cycle) const;

    void dfsTopological(std::type_index current, std::unordered_set<std::type_index>& visited,
...
#define REGISTER_COMPONENT_DEPENDENCY(ComponentType, RequiredType, description)        \
    static struct ComponentType##RequiredType##Registration {                          \
        ComponentType##RequiredType##Registration() {                                  \
            Engine::ComponentDependencyRegistry::getInstance()                         \
                .registerRequiredDependency<ComponentType, RequiredType>(description); \
            Engine::ComponentDependencyRegistry::getInstance().registerComponentName(  \
                typeid(ComponentType), #ComponentType);                                \
        }                                                                              \
