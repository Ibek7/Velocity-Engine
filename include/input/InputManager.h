...
public:
    ...
    bool saveToFile(const std::string& filepath) const {
        ...
        file.write(reinterpret_cast<const char*>(&metadata.recordingDate), sizeof(metadata.recordingDate));
        file.write(reinterpret_cast<const char*>(&metadata.totalDuration), sizeof(metadata.totalDuration));
        ...
        for (const auto& event : events) {
            ...
            file.write(reinterpret_cast<const char*>(&event.frameNumber), sizeof(event.frameNumber));
            file.write(reinterpret_cast<const char*>(&event.keyCode), sizeof(event.keyCode));
            file.write(reinterpret_cast<const char*>(&event.buttonIndex), sizeof(event.buttonIndex));
            ...
            file.write(reinterpret_cast<const char*>(&event.gamepadIndex), sizeof(event.gamepadIndex));
...
public:
    ...
    void recordMouseButtonEvent(int button, bool pressed, float x, float y, uint32_t frame) {
        ...
        RecordedInputEvent event;
        event.type = pressed ? RecordedInputType::MouseButtonDown : RecordedInputType::MouseButtonUp;
        event.timestamp = getRecordingTimestamp();
        event.frameNumber = frame;
    ...
    void setOnRecordingFinished(std::function<void(const InputRecording&)> cb) { onRecordingFinished = cb; }
    ...
    void setOnEventPlayback(std::function<void(const RecordedInputEvent&)> cb) { onEventPlayback = cb; }
...
public:
    ...
    void registerMacro(const InputMacro& macro) {
    ...
    void setOnMacroEvent(std::function<void(const RecordedInputEvent&)> cb) {
        onMacroEvent = cb;
    }
