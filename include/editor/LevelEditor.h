...
enum class EditorMode { Select, Move, Rotate, Scale, Paint, Erase };

enum class SnapMode { None, Grid, Object, Vertex };

enum class EditorTool { Selection, Move, Rotate, Scale, Brush, Eraser, Eyedropper, Hand };
...
struct PropertyValue {
    enum Type { Bool, Int, Float, String, Vector2, Color, Entity } type;
    ...
    PropertyValue(const Math::Vector2D& value) : type(Vector2) {
        new (&data.vector2Value) Math::Vector2D(value);
    }
    PropertyValue(const Graphics::Color& value) : type(Color) {
        new (&data.colorValue) Graphics::Color(value);
    }
    PropertyValue(ECS::Entity* value) : type(Entity) { data.entityValue = value; }

    PropertyValue(const PropertyValue& other) : type(other.type), stringValue(other.stringValue) {
        switch (type) {
            case Bool:
            ...
            case String:
                break;
            case Vector2:
                new (&data.vector2Value) Math::Vector2D(other.data.vector2Value);
                break;
            case Color:
                new (&data.colorValue) Graphics::Color(other.data.colorValue);
                break;
            case Entity:
                data.entityValue = other.data.entityValue;
                break;
        }
    }
...
struct EditorAction {
    ...
    EditorAction()
        : command(EditorCommand::None), timestamp(std::chrono::high_resolution_clock::now()) {}
};
